...
function deleteEnergyButton(id){
 ...
 if(s.energyButtons.length<=1){s.feedback="Hãy tạo thêm một nút trước khi xóa nút cuối cùng.";save(s);render("energy");openEnergySettings(id);return;}
 s.energyButtons.splice(i,1);s.feedback="Đã xóa nút. Lịch sử ghi nhận trước đây vẫn được giữ.";save(s);render("energy");openEnergySettings(s.energyButtons[Math.min(i,s.energyButtons.length-1)].id);
}
