...
function energyQuickMarkup(s,returnTab){
 ...
 return'<section class="rpg-panel energy-quick"><div class="rpg-head"><h2>Năng lượng</h2><button type="button" class="btn-ghost" data-open-energy>⚡</button></div><div class="energy-quick-grid">'+s.energyButtons.map(b=>{const left=energyCooldownRemaining(s,b.id);return'<button type="button" class="btn-ghost energy-action" data-energy-quick="'+esc(b.id)+'"'+(left>0?' disabled':'')+'><span class="energy-icon">'+esc(energyIcon(b))+'</span><span>'+esc(b.title)+'</span><small class="energy-cooldown-label" data-energy-cooldown="'+esc(b.id)+'"'+(left>0?'':' hidden')+'>'+(left>0?'Hồi chiêu '+formatEnergyCooldown(left):'')+'</small></button>';}).join("")+'</div></section>';
}
