...
function recordEnergy(buttonId){
...
function renderEnergy(s){
 ...
 const counts={};events.forEach(e=>counts[e.title]=(counts[e.title]||0)+1);const labels={day:"Hôm nay",hour:"Giờ này",week:"Tuần này"},icons=["✨","⚡","🪫","💧","🍎","🏃","🧘","📚","❤️","🌙","🔥","🎯"];
 const iconOptions=icons.map(x=>'<option value="'+x+'">'+x+'</option>').join("");
 const editors=s.energyButtons.map(b=>'<form class="energy-edit-row" data-edit-energy="'+esc(b.id)+'"><label>Icon<select name="icon">'+icons.map(x=>'<option value="'+x+'" '+((b.icon||"✨")===x?"selected":"")+'>'+x+'</option>').join("")+'</select></label><label>Mô tả<input name="title" maxlength="60" required value="'+esc(b.title)+'"></label><div class="rpg-grid">'+KEYS.map(k=>'<label>'+k+'<select name="'+k+'"><option value="0" '+(!(Number((b.effects||{})[k]))?"selected":"")+'>0</option><option value="1" '+(Number((b.effects||{})[k])===1?"selected":"")+'>+1</option><option value="-1" '+(Number((b.effects||{})[k])===-1?"selected":"")+'>&minus;1</option></select></label>').join("")+'</div><div class="rpg-actions"><button class="btn-primary">Lưu thay đổi</button>'+(b.custom?'<button type="button" class="btn-ghost" data-delete-energy="'+esc(b.id)+'">Xóa nút</button>':"")+'</div></form>').join("");
 view.innerHTML=style()+'<div class="rpg-wrap"><section class="rpg-panel"><div class="rpg-head"><h2>Năng lượng</h2><button type="button" id="energy-settings-toggle" class="btn-ghost" aria-expanded="false" aria-controls="energy-settings">⚙️ Cài đặt</button></div><p class="rpg-muted">Chạm nút để ghi nhận.</p><div class="rpg-grid">'+s.energyButtons.map(b=>'<button type="button" class="btn-ghost energy-action" data-energy="'+esc(b.id)+'"><span class="energy-icon">'+esc(b.icon||"✨")+'</span><span>'+esc(b.title)+'</span></button>').join("")+'</div></section><section class="rpg-panel"><h2>Tần suất · '+labels[mode]+'</h2><label>Xem theo <select id="energy-view"><option value="day" '+(mode==="day"?"selected":"")+'>Ngày</option><option value="hour" '+(mode==="hour"?"selected":"")+'>Giờ</option><option value="week" '+(mode==="week"?"selected":"")+'>Tuần</option></select></label><p><b>'+events.length+'</b> lần ghi nhận trong khoảng đã chọn.</p><div class="rpg-manage-list">'+(Object.keys(counts).length?Object.keys(counts).map(k=>'<div class="rpg-manage-row"><span>'+esc(k)+'</span><strong>'+counts[k]+' lần</strong></div>').join(""):'<div class="rpg-empty">Chưa có ghi nhận.</div>')+'</div><h3>Lịch sử</h3><div class="rpg-history">'+(events.length?events.slice(0,150).map(e=>'<div><b>'+esc(e.date)+'</b> · '+esc(new Date(e.at).toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"}))+' · '+esc(e.title)+' · '+esc(taskEffectsLine(e.statDelta))+'</div>').join(""):'<div class="rpg-muted">Chưa có ghi nhận.</div>')+'</div></section><section class="rpg-panel"><h2>Tạo nút mới</h2><form id="energy-button-form" class="rpg-form"><label>Icon<select name="icon">'+iconOptions+'</select></label><label>Mô tả nút<input name="title" maxlength="60" required placeholder="Ví dụ: Đi bộ"></label><div class="rpg-grid">'+KEYS.map(k=>'<label>'+k+'<select name="'+k+'"><option value="0">0</option><option value="1">+1</option><option value="-1">&minus;1</option></select></label>').join("")+'</div><button class="btn-primary">Thêm nút</button></form></section><section class="rpg-panel energy-settings" id="energy-settings" hidden><h2>Cài đặt các nút</h2><p class="rpg-muted">Sửa biểu tượng, mô tả và ảnh hưởng chỉ số cho từng nút.</p><div class="rpg-manage-list">'+editors+'</div></section>'+(s.feedback?'<p class="rpg-feedback">'+esc(s.feedback)+'</p>':"")+'</div>';
 document.getElementById("energy-view").onchange=e=>{const url=new URL(location.href);url.searchParams.set("energyView",e.target.value);history.replaceState(null,"",url);render("energy");};
 document.getElementById("energy-settings-toggle").onclick=e=>{const panel=document.getElementById("energy-settings"),open=panel.hidden;panel.hidden=!open;e.currentTarget.setAttribute("aria-expanded",String(open));};
 document.querySelectorAll("[data-energy]").forEach(b=>b.onclick=()=>recordEnergy(b.dataset.energy));
 document.querySelectorAll("[data-delete-energy]").forEach(b=>b.onclick=()=>deleteEnergyButton(b.dataset.deleteEnergy));
 document.querySelectorAll("[data-edit-energy]").forEach(f=>f.onsubmit=e=>{e.preventDefault();const form=e.currentTarget,data={id:form.dataset.editEnergy,title:form.elements.title.value,icon:form.elements.icon.value};KEYS.forEach(k=>data[k]=form.elements[k].value);updateEnergyButton(data);});
