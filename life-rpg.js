...
function energyQuickMarkup(s,returnTab){
 ...
 return'<section class="rpg-panel energy-quick"><div class="rpg-head"><h2>Năng lượng</h2><button type="button" class="btn-ghost" data-open-energy>⚡</button></div><div class="energy-quick-grid">'+s.energyButtons.map(b=>'<button type="button" class="btn-ghost energy-action" data-energy-quick="'+esc(b.id)+'"><span class="energy-icon">'+esc(energyIcon(b))+'</span><span>'+esc(b.title)+'</span></button>').join("")+'</div></section>';
}
function bindEnergyQuick(returnTab){
 ...
 document.querySelectorAll("[data-open-energy]").forEach(b=>b.onclick=()=>render("energy"));
}
