...
function renderLuckySettings(s,target){
 const options=Array.from({length:6},(_,n)=>'<option value="'+n+'">'+(n===0?"0":"+"+n)+'</option>').join("");
 target.innerHTML=style()+'<div class="rpg-wrap"><section class="rpg-panel"><h3>Thưởng khi bấm Lucky</h3><p class="rpg-muted">Mỗi chỉ số có thể cộng từ 0 đến +5. Mỗi lần bấm được ghi trong Lịch sử XP & Stats.</p><form id="lucky-config-form" class="rpg-grid">'+KEYS.map(k=>'<label><b>'+k+'</b><select name="'+k+'">'+options+'</select></label>').join("")+'<button class="btn-primary" style="grid-column:1/-1">Lưu mức thưởng</button></form></section></div>';
...
function historyRecordText(e){
 ...
 return(e.date||"")+(timeText?" "+timeText:"")+" · "+(e.title||e.action||"Event")+" · "+(xp>0?"+"+xp:xp)+" XP"+(stats?" · "+stats:"");
}
async function copyHistoryToClipboard(){
 const text=["Mori Quest — Lịch sử XP & Stats","Tổng bản ghi: "+state().history.length,...state().history.slice().reverse().map(historyRecordText)].join("\n");
 try{await navigator.clipboard.writeText(text);return true;}catch(_){
  const area=document.createElement("textarea");area.value=text;area.setAttribute("readonly","");area.style.position="fixed";area.style.opacity="0";document.body.appendChild(area);area.select();let copied=false;try{copied=document.execCommand("copy");}catch(__){}area.remove();return copied;
 }
}
