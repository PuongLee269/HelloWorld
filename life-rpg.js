...
function renderEnergy(s){
 ...
 document.querySelectorAll("[data-energy]").forEach(b=>b.onclick=()=>recordEnergy(b.dataset.energy));document.querySelectorAll("[data-delete-energy]").forEach(b=>b.onclick=()=>deleteEnergyButton(b.dataset.deleteEnergy));
 const form=document.getElementById("energy-button-form");form.onsubmit=e=>{e.preventDefault();const f=e.currentTarget,data={title:f.elements.title.value};KEYS.forEach(k=>data[k]=f.elements[k].value);if(addEnergyButton(data))f.reset();};
}
