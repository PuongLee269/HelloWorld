...
function updateEnergyButton(form){const s=state(),button=s.energyButtons.find(b=>b.id===form.id);if(!button)return;const title=String(form.title||"").trim();if(!title){alert("Nhập tên nút.");return;}const effects={};KEYS.forEach(k=>{const n=Math.max(-1,Math.min(1,Math.round(Number(form[k])||0)));if(n)effects[k]=n;});button.title=title.slice(0,60);button.icon=String(form.icon||"✨").trim()||"✨";button.effects=effects;save(s);render("energy");openEnergySettings(button.id);}
function deleteEnergyButton(id){const s=state(),i=s.energyButtons.findIndex(b=>b.id===id&&b.custom);if(i<0)return;s.energyButtons.splice(i,1);save(s);render("energy");}
...
function clearEnergyHistory(){
 const s=state();s.energyEvents=[];s.history=s.history.filter(e=>e.action!=="energy_event");s.feedback="Đã xóa lịch sử tần suất Năng lượng; chỉ số hiện tại được giữ nguyên.";save(s);render("energy");
}
function renderEnergy(s){
 ...
 document.getElementById("energy-settings-toggle").onclick=()=>openEnergySettings();
